...
# Ler arquivo "deploy_list.txt" e extrair os diretórios
grep -o "\(.*/\)" deploy_list.txt > fdeploy_out.txt
...
for line in $(cat deploy_list.txt); do echo 'curl -o bkp/'$line' "ftp://'$USER':'$PASSWD'@'$HOST'/%2f'$DIRETORIO_REMOTO'/'$line'"' ; done > copiar_arquivos_bkp.sh
